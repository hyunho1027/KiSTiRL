...
public class DroneAgent : Agent
{
	...
	public override void AgentAction(float[] vectorAction, string textAction)
	{
		...
		rb1.AddForce(moter1.transform.up * (act0 * 50));
		rb2.AddForce(moter2.transform.up * (act1 * 50));
		rb3.AddForce(moter3.transform.up * (act2 * 50));
		rb4.AddForce(moter4.transform.up * (act3 * 50));
		...
		if ((goal.transform.position - gameObject.transform.position).magnitude < 0.5f)
		{
			...
			Mathf.Abs(gameObject.transform.position.y - droneInitPos.y) > 6f ||
